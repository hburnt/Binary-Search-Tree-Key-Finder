...
public:
    ...
    std::string findHelper(int key, Node* n) const;
private:
    Node* root;
...
std::string BinarySearchTree::find(int key) const {
    ...
    // Continue until there is a null pointer
    std::string value = findHelper(key, root);
    return value;
}
...
std::string BinarySearchTree::findHelper(int key, Node* n) const {
    ...
    if (key < n->key) return findHelper(key, n->left);

    if (key > n->key) return findHelper(key, n->right);
}
